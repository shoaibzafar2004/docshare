...
- **No invented persistence complexity.** Documents and shares live in three small SQLite tables with plain SQL (no ORM). Given the scope (a handful of tables, no migrations needed yet), an ORM would have added setup and abstraction without paying for itself.
